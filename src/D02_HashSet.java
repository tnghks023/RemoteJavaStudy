...
public class D02_HashSet {
	...
	public static void main(String[] args) {
		...
		// 리스트 컬렉션은 아주 쉡게 크기순으로 정렬할 수 있다
		Collections.sort(number_list);
		System.out.println("정렬 후 : " + number_list);

		// Collection.addAll(Collection) 컬렉션의 모든 내용을 다른 컬렉션에 추가하기(합집합)
		...
		// Collection.removeAll(Collection) : 전달한 컬렉셔과 같은 내용을 모두 제거(차집합)

		animals.removeAll(animal_list);

		System.out.println(animals);
		
		//Collection.retainAll(Collection) : 두 컬렉션에 모두 존재하는 값만 남김(교집합)
